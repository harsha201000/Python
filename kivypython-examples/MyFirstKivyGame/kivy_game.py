...
class GameWidget(Widget):
    def __init__(self,**kwargs):
        ...
        self._scoreLabel = CoreLabel(text="Score : 0",font_size=50)
        self._scoreLabel.refresh()
        self._score = 0
        ...
        with self.canvas:
            ...
            self._score_instruction = Rectangle(texture=self._scoreLabel.texture,pos=(0,Window.height-50),size=self._scoreLabel.texture.size)
    ...
    def spawn_enemies(self,distance):
        for i in range(5):
    ...
    def _on_frame(self,distance):
        self.dispatch("on_frame",distance)
        
    def on_frame(self,distance):
        pass
    ...
    def score(self,value):
        self._score = value
        self._scoreLabel.text = "Score: " + str(value)
        self._scoreLabel.refresh()
        self._score_instruction.texture = self._scoreLabel.texture
        self._score_instruction.size = self._scoreLabel.texture.size
    ...
    def colliding_entities(self,entity):
        ...
        for e in self._entities:
            if self.collidies(e,entity) and e != entity:
                result.add(e)
        return result
...
class Bullet(Entity):
    ...
    def move_step(self,sender,distance):
        ...
        # move
        step_size = self._speed * distance
        new_x = self.pos[0]
...
class Player(Entity):
    ...
    def shoot_step(self,distance):
        # shoot
        if "spacebar" in game.keys_pressed:
            x = self.pos[0]
            y = self.pos[1] + 50
    ...
    def move_step(self,sender,distance):
        # move
        step_size = 200 * distance
        new_x = self.pos[0]
        new_y = self.pos[1]
        if "left" in game.keys_pressed:
            new_x -= step_size
        if "right" in game.keys_pressed:
            new_x += step_size
        self.pos = (new_x,new_y)
...
class MyFirstKivyGameApp(App):
    def build(self):
        self.icon = "game.png"
        return game
    
if __name__ == "__main__":
    app = MyFirstKivyGameApp()
    app.run()
