...
# We will work with vector 2 becaus eit has some useful functions
                    ...
                    try:
                        if self.eatable:
                            if self.position.distance_to(i.position) < self.radius + i.radius:
                                ...
                                planets.remove(self)
                        dir_from_obj = (i.position - self.position).normalize() * 0.01
...
# Two Suns and two opposing planets
...
for x in range(gridDimension):
    for y in range(gridDimension):
        Planet(Vector2(gridgap * x + 40, gridgap * y + 40), radius=3, immovable=True)
