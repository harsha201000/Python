...
while True:
    ...
    pygame.display.set_caption("Carroms: WHITE(%s) vs BLACK(%s)" % (player1, player2))
    ...
    def handle_user_input(win, carrom, permit_rotation=False):
        ...
        while get_user_input:
            ...
            carrom.striker.velocity.from_polar(
                (striker_speed, -90-striker_angle if carrom.player_turn == 0 else 90+striker_angle))
            ...
            carrom.draw(win)
            carrom.board.show_notification(win, "WHITE'S TURN" if carrom.player_turn == 0 else "BLACK'S TURN")
